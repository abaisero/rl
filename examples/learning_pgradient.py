...
def target_pbar(nruns, nepisodes, nprocesses, q):
    pbar_runs = tqdm(desc='Total Runs', total=nruns, position=0)
    pbar_episodes = dict()
    for type_, name in iter(q.get, None):
        try:
            pos, pbar_episode = pbar_episodes[name]
        except KeyError:
            pos = len(pbar_episodes) + 1
            pbar_episode = tqdm(desc=name, total=nepisodes, position=pos)
            pbar_episodes[name] = pos, pbar_episode
...
if __name__ == '__main__':
    ...
    def run(ri, q=None):
        ...
        for e in range(nepisodes):
            ...
            returns_run[idx_returns] = ovalues.sum()
    ...
    if nprocesses > 1:  # parallel
        with mp.Pool(processes=nprocesses, initializer=initializer) \
                as pool:
